...
// FULL CODE 100% - PROFILE, CHAT, FOLLOW, EDIT PROFILE
// =============================================
...
(function() {
    ...
    script.onload = function() {
        if (typeof window.VConsole !== 'undefined') {
            new window.VConsole();
            console.log('[Dunukha] vConsole ready');
        }
    };
...
// SAFE IMAGE
// =============================================
function createSafeImage(src, fallback, cls, style) {
...
function safeShare(title, text, url) {
    if (navigator.share) {
        navigator.share({ title: title, text: text, url: url }).catch(function(e) { console.log('Share canceled'); });
    } else {
...
function setupAuth() {
    ...
    document.getElementById('email-login-btn').onclick = async function() {
        ...
        try { await signInWithEmailAndPassword(auth, em, pw); } catch (e) { showToast(e.message); }
    };
    document.getElementById('email-register-btn').onclick = async function() {
        var em = document.getElementById('login-email').value;
        var pw = document.getElementById('login-password').value;
        try { await createUserWithEmailAndPassword(auth, em, pw); } catch (e) { showToast(e.message); }
    };
        ...
        if (user) {
            ...
            if (!snap.exists()) {
                await setDoc(ref, {
                    ...
                    email: user.email || '', photoURL: user.photoURL || DEFAULT_AVATAR,
                    username: user.email ? user.email.split('@')[0] : 'user'+user.uid.slice(0,6),
                    bio: '', location: '', followers: {}, following: {}, savedPosts: [], createdAt: serverTimestamp()
                });
            }
            navigateTo('home');
            listenNotifications();
...
function navigateTo(page) {
    ...
    var a = document.querySelector('.bottom-nav i[data-page="' + page + '"]');
...
async function loadStories() {
    ...
    for (var uid in map) {
        ...
        var av = createSafeImage(u.photoURL, DEFAULT_AVATAR, '', 'width:100%;height:100%;border-radius:50%;border:2px solid white;object-fit:cover;');
...
function viewStory(stories) {
    ...
    var img = document.createElement('img'); img.style.cssText = 'width:100%;height:100%;object-fit:contain;'; img.onerror = function() { this.src = DEFAULT_POST_IMAGE; };
...
async function loadComments(postId) {
    var c = document.getElementById('comments-'+postId); if (!c) return;
    var snap = await getDoc(doc(db,"posts",postId)); var comments = snap.data().comments||[]; c.innerHTML = '';
    comments.forEach(function(cm) { var d=document.createElement('div'); d.style.cssText='padding:5px 0;border-bottom:1px solid #f5f5f5;display:flex;gap:8px;'; var av=createSafeImage(cm.avatar,DEFAULT_AVATAR,'','width:24px;height:24px;border-radius:50%;'); d.appendChild(av); var td=document.createElement('div'); td.innerHTML='<strong>'+cm.username+'</strong> '+cm.text; d.appendChild(td); c.appendChild(d); });
}
...
async function loadUserProfile(targetUid) {
    ...
    container.innerHTML = `
        <div class="profile-header">
            <img class="profile-avatar" src="${profile.photoURL || DEFAULT_AVATAR}" onerror="this.src='${DEFAULT_AVATAR}';">
            <div style="flex:1;">
                ...
                <div id="profile-action-area" style="margin-top:10px;display:flex;gap:8px;">
                    ${isOwner ? '<button class="profile-action-btn" id="edit-profile-btn">Chỉnh sửa hồ sơ</button>' : `
                        <button class="profile-action-btn ${following ? 'following' : 'primary'}" id="follow-btn">${following ? 'Đang theo dõi' : 'Theo dõi'}</button>
                        <button class="profile-action-btn primary" id="message-btn">Nhắn tin</button>
                    `}
                </div>
            </div>
        </div>
    ...
    if (isOwner) {
        ...
        document.getElementById('message-btn').onclick = function() {
            openChatWithUser(targetUid, profile.displayName);
        };
    }
}
...
document.getElementById('save-profile-btn').onclick = async function() {
    ...
    if (file) {
        try { updates.photoURL = await uploadImgBB(file); updates.avatarUrl = updates.photoURL; } catch(e) { return showToast('Lỗi upload'); }
    }
...
document.getElementById('edit-avatar-input').onchange = function() {
    var file = this.files[0];
    if (file) { var reader = new FileReader(); reader.onload = function(e) { var prev = document.getElementById('edit-avatar-preview'); prev.src = e.target.result; prev.style.display = 'block'; }; reader.readAsDataURL(file); }
};
...
function openChatWithUser(uid, name) {
    ...
    unsubscribes['chat'] = onSnapshot(query(collection(db, "messages"), where("chatId", "==", chatId), orderBy("createdAt")), function(snap) {
        var mc = document.getElementById('chat-messages'); mc.innerHTML = '';
        snap.forEach(function(doc) { var d=doc.data(); var row=document.createElement('div'); row.className='message-row '+(d.from===currentUser.uid?'sent':'received'); row.innerHTML='<div class="message-bubble">'+d.text+'</div>'; mc.appendChild(row); });
...
async function loadChatList() {
    ...
    for (var uid in following) {
        ...
        var u = await getDoc(doc(db, "users", uid)); if (!u.exists()) continue; var d = u.data();
        var div = document.createElement('div'); div.style.cssText = 'display:flex;align-items:center;gap:10px;padding:10px;cursor:pointer;';
        var av = createSafeImage(d.photoURL, DEFAULT_AVATAR, '', 'width:44px;height:44px;border-radius:50%;');
        div.appendChild(av); var sp = document.createElement('span'); sp.textContent = d.displayName; div.appendChild(sp);
        div.onclick = function() { openChatWithUser(uid, d.displayName); }; list.appendChild(div);
    }
}
...
function setupSearch() {
    ...
    input.addEventListener('input', async function() {
        ...
        filtered.forEach(function(u){var div=document.createElement('div');div.className='search-result';var av=createSafeImage(u.data.photoURL,DEFAULT_AVATAR,'','width:44px;height:44px;border-radius:50%;');var info=document.createElement('div');info.innerHTML='<strong>'+u.data.displayName+'</strong><br><span style="color:#888;">@'+u.data.username+'</span>';div.appendChild(av);div.appendChild(info);div.onclick=function(){loadUserProfile(u.id);navigateTo('profile');};results.appendChild(div);});
    });
}
async function loadExplore() { var grid = document.getElementById('search-results'); grid.style.display='grid'; grid.style.gridTemplateColumns='repeat(3,1fr)'; grid.style.gap='3px'; grid.innerHTML=''; var snap = await getDocs(query(collection(db,"posts"),orderBy("createdAt","desc"),limit(12))); snap.forEach(function(doc){var d=doc.data();var img=createSafeImage(d.mediaUrls?d.mediaUrls[0]:(d.mediaUrl||''),DEFAULT_POST_IMAGE,'','width:100%;aspect-ratio:1;object-fit:cover;cursor:pointer;');img.onclick=function(){loadUserProfile(d.uid);navigateTo('profile');};grid.appendChild(img);}); }
