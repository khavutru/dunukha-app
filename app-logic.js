...
// ĐÃ SỬA TRIỆT ĐỂ LỖI ẢNH CHẾT - ONERROR 100%
// FULL CODE - KHÔNG VIẾT TẮT
// =============================================
...
// =============================================
// HÀM TẠO ẢNH AN TOÀN VỚI ONERROR TỰ ĐỘNG
// =============================================
function safeImg(src, fallback, className, extraStyle){
    var img = document.createElement('img');
    img.src = src || fallback;
    if(className) img.className = className;
    if(extraStyle) img.style.cssText = extraStyle;
    img.setAttribute('onerror', "this.onerror=null; this.src='" + fallback + "';");
...
function viewStory(stories){
    ...
    var img=document.createElement('img');img.style.cssText='width:100%;height:100%;object-fit:contain;';img.setAttribute('onerror',"this.onerror=null; this.src='"+DEFAULT_POST+"';");img.onerror=function(){this.src=DEFAULT_POST;this.onerror=null;};
    ...
    if(post.mediaUrls&&post.mediaUrls.length){
        post.mediaUrls.forEach(function(url){
            if(url.match(/\.(mp4|webm|ogg|mov)$/i)){
                ...
                media+='<img class="post-media" src="'+url+'" loading="lazy" onerror="this.onerror=null; this.src=\''+DEFAULT_POST+'\';">';
            }
        });
    }else{
        media='<img class="post-media" src="'+(post.mediaUrl||post.postUrl||DEFAULT_POST)+'" onerror="this.onerror=null; this.src=\''+DEFAULT_POST+'\';">';
    }
...
async function loadComments(postId){
    ...
    comments.forEach(function(cm){
        var d=document.createElement('div');d.style.cssText='padding:5px 0;border-bottom:1px solid #f5f5f5;display:flex;gap:8px;';
        var av=safeImg(cm.avatar, DEFAULT_AVATAR, '', 'width:24px;height:24px;border-radius:50%;');
        d.appendChild(av);
        var td=document.createElement('div');td.innerHTML='<strong>'+cm.username+'</strong> '+cm.text;
        d.appendChild(td);c.appendChild(d);
    });
}
...
async function loadProfilePosts(uid){
    ...
    snap.forEach(function(doc){
        var d=doc.data();
        var img=safeImg(d.mediaUrls?d.mediaUrls[0]:(d.mediaUrl||''), DEFAULT_POST, '', 'width:100%;aspect-ratio:1;object-fit:cover;cursor:pointer;');
...
function setupSearch(){
    ...
    input.addEventListener('input',async function(){
        ...
        filtered.forEach(function(u){
            ...
            var av=safeImg(u.data.photoURL, DEFAULT_AVATAR, '', 'width:44px;height:44px;border-radius:50%;');
            var info=document.createElement('div');info.innerHTML='<strong>'+u.data.displayName+'</strong><br><span style="color:#888;">@'+u.data.username+'</span>';
            div.appendChild(av);div.appendChild(info);
            div.onclick=function(){loadUserProfile(u.id);navigateTo('profile');};
...
// =============================================
// EXPLORE (ĐÃ CÓ ONERROR)
// =============================================
async function loadExplore(){
    var grid=document.getElementById('search-results');
    grid.style.display='grid';grid.style.gridTemplateColumns='repeat(3,1fr)';grid.style.gap='3px';grid.innerHTML='';
    var snap=await getDocs(query(collection(db,"posts"),orderBy("createdAt","desc"),limit(12)));
    snap.forEach(function(doc){
        var d=doc.data();
        var img=safeImg(d.mediaUrls?d.mediaUrls[0]:(d.mediaUrl||''), DEFAULT_POST, '', 'width:100%;aspect-ratio:1;object-fit:cover;cursor:pointer;');
        img.onclick=function(){loadUserProfile(d.uid);navigateTo('profile');};
        grid.appendChild(img);
    });
}

// =============================================
// REELS, NOTIFICATIONS, SIDEBAR
// =============================================
...
setupPost();
setupSearch();
console.log('[Dunukha] Hệ thống đã sẵn sàng! Tất cả ảnh đã có onerror bảo vệ.');
